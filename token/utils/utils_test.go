...
import (
	"github.com/everFinance/everpay-go/token/schema"
	"math/big"
	"testing"
...
func TestTag(t *testing.T) {
	assert.Equal(t, "ethereum-usdt-0xabc", Tag(schema.ChainTypeEth, "USDT", "0xABC"))
	assert.Equal(t, "err_invalid_token", Tag(schema.ChainTypeCrossArEth, "AR", "0xABC"))
	assert.Equal(t, "arweave,ethereum-ar-ABC,0xabc", Tag(schema.ChainTypeCrossArEth, "AR", "ABC,0xABC"))
}
...
func TestToken_SplitTargetAmount(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, "0", amount.String()) // return 0, because diffDecimal = -4, amount == 0.001 but amount must integer so is 0
...
func TestToken_CombineBurnAmount(t *testing.T) {
	...
	everTokenDecimal = 60
	targetAmount, ok := new(big.Int).SetString("10000000000000000000000000000000000000000000000000000000000001", 10)
	assert.Equal(t, true, ok)

	amount, err = ZoomBurnAmount(everTokenDecimal, 0, targetAmount)
	assert.Equal(t, `everAmount zoom can not be float type; amount: 10.000000000000000000000000000000000000000000000000000000000001`, err.Error())
