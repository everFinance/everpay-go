...
import (
	cacheSchema "github.com/everFinance/everpay-go/cache/schema"
	"github.com/everFinance/everpay-go/sdk/schema"
	serverSchema "github.com/everFinance/everpay-go/server/schema"
	"sync"
	"time"
)
...
func (s *SubscribeTx) run() {
	...
	for {
		...
		case <-t1.C:
			if isAccTxs {
				txs, err = s.client.CursorTxsByAcc(s.filterQuery.Address, cursorId, s.filterQuery.TokenSymbol, s.filterQuery.Action, s.filterQuery.WithoutAction)
			} else {
				txs, err = s.client.CursorTxs(cursorId, s.filterQuery.TokenSymbol, s.filterQuery.Action, s.filterQuery.WithoutAction)
			}
